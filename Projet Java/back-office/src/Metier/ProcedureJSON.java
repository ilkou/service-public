...
public class ProcedureJSON {
    public static void CreatJSonfile()
    {
        ...
        File f = new File(Path + "ProcedureJsonfile.json");
        f.delete();
        int i=0;
        while(i < array.size()){
            ...
            jsonObject.put("descDocuments",nom);
            jsonObject.put("descDocumentsDescription",desc);
            jsonObject.put("nbrEtapes",array.get(i).getEtape().size());
            jsonObject.put("etapeDoc",list);

            try {
                FileWriter file = new FileWriter(Path + "ProcedureJsonfile.json",true);
                if(i == 0){
                    file.append("{ \n procedures: [ \n");
