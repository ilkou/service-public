...
public class DemandeDAO {
	...
	public void insertOldDemande(Demande demande) {
		...
		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(unset("etat")));
		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(set("etat", demande.getEtat().toString())));

		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(unset("DocsPath")));
		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(set("DocsPath", demande.getAllDocsPath())));

		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(unset("procedure")));
		ConnectToDB.getMyCollection().updateOne(eq("_id", new ObjectId(demande.getJeton())),
				combine(set("procedure", new Document("nom", demande.getProcedure().getNom())
